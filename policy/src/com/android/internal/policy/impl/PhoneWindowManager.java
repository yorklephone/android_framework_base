...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    static public final String SYSTEM_DIALOG_REASON_LOCKSCREEN = "lockscreen";
    ...
    // Behavior of Magic wake
    
    boolean mMagicWakeScreen;
    ...
    /**
     *      * When a recent-key press expires, close other system windows and launch the recent apps
     */
    Runnable mRecentPress = new Runnable() {
        public void run() {
            if (Settings.System.getInt(mContext.getContentResolver(),
                    Settings.System.SHOW_RECENT_APP_DIALOG, 0) == 0) {
    ...
    Runnable mDownPress = new Runnable() {
        public void run() {
        	if (Settings.System.getInt(mContext.getContentResolver(),
                    Settings.System.DOWN_PRESS_LOCK_SCREEN, 1) == 1) {
    ...
    Runnable mMagicPress = new Runnable() {
        public void run() {
            if (Settings.System.getInt(mContext.getContentResolver(),
                    Settings.System.MAGIC_PRESS_WAKE_SCREEN, 0) == 0) {
                // Bail out unless the user has elected to turn this on.
                return;
            }
            performHapticFeedbackLw(null, HapticFeedbackConstants.LONG_PRESS, false);
    ...
    /**
     * When Magic-key longpress expires, dosomething
     */
    Runnable mMagicLongPress = new Runnable() {
        public void run() {
            // set the long press flag to true
            mIsLongPress = true;
            // Call camera activity or lunch sencend apps...
            sendMediaButtonEvent(KeyEvent.KEYCODE_MEDIA_NEXT);
        };
    };
    ...
    public void updateSettings() {
        ...
        synchronized (mLock) {
            ...
            mMagicWakeScreen = (Settings.System.getInt(resolver,
                            Settings.System.MAGIC_PRESS_WAKE_SCREEN, 0) == 1);
...
//                mLidOpen = sw == 0;            
    ...
    public boolean interceptKeyBeforeDispatching(WindowState win, int action, int flags,
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
            ...
            if (down && repeatCount == 0) {
                ...
                return true;
       } else if (keyCode == KeyEvent.KEYCODE_LEFT_RIGHT) {
            if (down && repeatCount == 0) {
                mHandler.post(mRecentPress);
            }
            return true;
       } else if (keyCode == KeyEvent.KEYCODE_MENU_D) {
            if (down && repeatCount == 0) {
                mHandler.post(mDownPress);
            }
            return true;
       } else if (keyCode == KeyEvent.KEYCODE_MAGIC_VIEW) {
            if (down && repeatCount == 0) {
                mHandler.post(mMagicPress);
            }
            return true;
    ...
    void handleMagicLongPress(int keycode) {
        
        Runnable mMagicHandler;
        mMagicHandler = mMagicLongPress;
        mHandler.postDelayed(mMagicHandler, ViewConfiguration.getLongPressTimeout());
    }
    ...
    public int interceptKeyBeforeQueueing(long whenNanos, int action, int flags,
        ...
        if (isScreenOn || isInjected) {
            ...
            boolean isWakeKey = (policyFlags
                    ...
                    || ((keyCode == KeyEvent.KEYCODE_MAGIC_VIEW) && mMagicWakeScreen)
            ...
            if (isWakeKey
                    && (!mMagicWakeScreen || isOffByProx)
                    && ((keyCode == KeyEvent.KEYCODE_MAGIC_VIEW))) {
                isWakeKey = false;
            }
            ...
            if(mMagicWakeScreen && isWakeKey && ((keyCode == KeyEvent.KEYCODE_MAGIC_VIEW)))
                keyCode = KeyEvent.KEYCODE_POWER;
    ...
    public int rotationForOrientationLw(int orientation, int lastRotation,
        ...
        synchronized (mLock) {
            ...
            //or case.unspecified
            if (mLidOpen) {
                //Log.w(TAG,"return mLidOpenRotation");
                return mLidOpenRotation;
            } else if (mDockMode == Intent.EXTRA_DOCK_STATE_CAR && mCarDockRotation >= 0) {
                //Log.w(TAG,"return mCarDockRotation");
                return mCarDockRotation;
            } else if (mDockMode == Intent.EXTRA_DOCK_STATE_DESK && mDeskDockRotation >= 0) {
                //Log.w(TAG,"retrun mDeskDockRotation");
                return mDeskDockRotation;
            } else {
                if (useSensorForOrientationLp(orientation)) {
                    //Log.w(TAG,"return mOrientationListener.getCurrentRotation(lastRotation)");
                    return mOrientationListener.getCurrentRotation(lastRotation);
                }
                //Log.w(TAG,"return Surface.ROTATION_0");
                return Surface.ROTATION_0;
            }
        }
    }
