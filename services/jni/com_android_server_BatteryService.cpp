...
#define BATTERY_ENABLE_DOCKING_BOOSTER_VOLTAG 4200

#define MSC_MAX_ORI             0x07
#define MSC_DOCK_BOOSTER        0xB  /* Docking Command 2 Booster */

#define PRIVATE_SYS
#ifdef PRIVATE_SYS
#define DOCK_EVENT_PATH "/sys/private/docking/event10"
#else
#define DOCK_EVENT_PATH "/dev/input/event10"
#endif

#define DOCKING_BOOSTER_DISABLE 1
#define DOCKING_BOOSTER_ENABLE  0
...
static void android_server_BatteryService_update(JNIEnv* env, jobject obj)
{
    ...
    setBooleanField(env, obj, gPaths.usbOnlinePath, gFieldIds.mUsbOnline);
 	setBooleanField(env, obj, gPaths.dockingOnlinePath, gFieldIds.mDockingOnline);
...
int register_android_server_BatteryService(JNIEnv* env)
{
    ...
    while ((entry = readdir(dir))) {
        ...
        if (length > 0) {
            ...
            else if (strcmp(buf, "DOCKING") == 0) {
                snprintf(path, sizeof(path), "%s/%s/online", POWER_SUPPLY_PATH, name);
                if (access(path, R_OK) == 0)
                    gPaths.dockingOnlinePath= strdup(path);
            }
    ...
    if (!gPaths.dockingOnlinePath)
        LOGE("dockingOnlinePath not found");
    ...
    gFieldIds.mDockingOnline = env->GetFieldID(clazz, "mDockingOnline", "Z");
    ...
    LOG_FATAL_IF(gFieldIds.mDockingOnline == NULL, "Unable to find BatteryService.DOCKING_ONLINE_PATH");
