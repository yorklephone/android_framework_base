...
//merge by yejianchun to froyo at 2010 11.01
/* linda add for service state */
import static com.android.internal.telephony.TelephonyProperties.PROPERTY_SERVICE_STATE;
//end
...
final class CdmaServiceStateTracker extends ServiceStateTracker {
    ...
    @Override
    protected void setPowerStateToDesiredWnc() {
        // If we want it on and it's off, turn it on
        if (mDesiredPowerState
            && cm.getRadioState() == CommandsInterface.RadioState.RADIO_OFF) {
            cm.setRadioPowerOffWnc(null);
        } else if (!mDesiredPowerState && cm.getRadioState().isOn()) {
            DataConnectionTracker dcTracker = phone.mDataConnection;
            if (! dcTracker.isDataConnectionAsDesired()) {
                ...
                EventLog.List val = new EventLog.List(
                        dcTracker.getStateInString(),
                        (dcTracker.getAnyDataEnabled() ? 1 : 0) );
                EventLog.writeEvent(EVENT_LOG_DATA_STATE_RADIO_OFF, val);
            }
            Message msg = dcTracker.obtainMessage(DataConnectionTracker.EVENT_CLEAN_UP_CONNECTION);
            msg.arg1 = 1; // tearDown is true
            msg.obj = CDMAPhone.REASON_RADIO_TURNED_OFF;
    ...
    protected void handlePollStateResult (int what, AsyncResult ar) {
        ...
        if (ar.exception != null) {
            ...
            case EVENT_POLL_STATE_OPERATOR_CDMA: // Handle RIL_REQUEST_OPERATOR
				...
				//add by yejianchun for modify get network service like china telcom 
				if(opNames[2].equals("46003") || opNames[2].equals("46005")) {
									opNames[0] = "China Telcom";
									opNames[1] = "CT";
				}
				//end
